...
printf "CEPH CLUSTER STATES:\n"
printf "ceph health: %s|" $($ceph_bin health | awk '{print $1}')
printf "active mon: %s|" $(ceph_mon_get_active) 
printf "osd_up_persent: %%%s |" $(ceph_osd_up_percent)
printf "osd_in_persent: %%%s |" $(ceph_osd_in_percent)
printf "degraded_percent: %%%s|" $pggdegraded
printf "pgtotal: %s\n" $pgtotal

printf "PG DETAIL STATES:\n"
printf "creating: %s|" $creating
printf "active: %s|" $active
printf "clean: %s|" $clean
printf "down: %s|" $down
printf "replay: %s|" $replay
printf "splitting: %s|" $splitting
printf "scrubbing: %s|" $scrubbing
printf "degraded: %s|" $degraded
printf "inconsistent: %s|" $inconsistent
printf "peering: %s|" $peering
printf "repair: %s|" $repair
printf "recovering: %s|" $recovering
printf "backfill: %s|" $backfill
printf "waitBackfill: %s|"  $waitBackfill
printf "incomplete: %s|" $incomplete
printf "stale: %s|" $stale
printf "remapped: %s\n" $remapped

printf "IOPS DETAIL STATES:\n"
printf "op_per_sec: %s|" $ops
printf "write_Bytes_sec: %s B\s|" $wrbps
printf "read_Bytes_sec: %s B\s\n" $rdbps
