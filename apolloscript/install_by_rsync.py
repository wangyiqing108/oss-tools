...
class DcmdInstall():
  def __init__(self):
    ...
    self.svr_pkg_md5_ = os.environ.get('DCMD_SVR_VERSION_MD5')
    self.svr_pkg_passwd_ = os.environ.get('DCMD_SVR_VERSION_PASSWD')
    ...
    self.rsync_user_ = None
    self.rsync_passwd_ = None
    ...
    self.svr_history_pkg_path_ = os.path.join(self.install_path_, "history","pkg")
    ...
    self.rsync_passwd_file_ = os.path.join(self.install_path_, "passwd")
    self.svr_tmp_path_ = os.path.join(self.install_path_, "tmp")
    self.svr_tmp_pkg_path_ = os.path.join(self.install_path_, "tmp","pkg")
    self.svr_tmp_env_path_ = os.path.join(self.install_path_, "tmp","env")
    self.svr_online_version_ = ""
    self.svr_online_env_version_ = ""
    self.svr_online_pkg_md5_ = ""
    self.svr_online_env_md5_ = ""
    self.svr_packages_ = []
    self.install_errmsg_ = ""
    self.install_success_ = False
    self.svr_pkg_file_ = "pkg_%s_%s_%s_%s.tar.gz" % (self.app_name_, self.svr_name_, self.svr_version_, self.svr_pkg_md5_[0:16])
    self.svr_pkg_pathfile_ = "%s/%s/pkg/%s" % (self.app_name_, self.svr_name_, self.svr_pkg_file_)
  ...
  def init_service_env(self):
    ...
    """
    #初始化环境
    prepare_cmd = self.svr_script_ + " prepare " + self.svr_home_
    ...
    if len(self.svr_user_) and cur_user <> self.svr_user_:
      print("Change user to:%s, current user:%s" % (self.svr_user_, cur_user))
      uid = pwd.getpwnam(self.svr_user_)
    ...
    print("Check service package path:%s" % self.svr_history_pkg_path_)
    self._check_and_mk_missed_path(self.svr_history_pkg_path_)
    print("Check service env path:%s" % self.svr_history_env_path_)
    self._check_and_mk_missed_path(self.svr_history_env_path_)
    print("Check pkg path:%s" % self.svr_tmp_pkg_path_)
    self._check_and_mk_missed_path(self.svr_tmp_pkg_path_)
    ...
    self.remove_outdate_pkg(self.svr_history_pkg_path_)
  ...
  def save_version_file(self):
    """ 保存版本信息
    """
    print("Save version[pkg:%s, pkg_md5:%s, env:%s, env_md5:%s] to %s" % 
          (self.svr_version_, self.svr_pkg_md5_, self.svr_env_v_, self.svr_env_md5_, self.svr_version_file_))
    conf = "[version]\npkg=%s\npkg_md5=%s\nenv=%s\nenv_md5=%s\n" % (self.svr_version_, self.svr_pkg_md5_, self.svr_env_v_, self.svr_env_md5_)
  ...
  def load_version_file(self):
    ...
    try:
        ...
        self.svr_online_version_ = parser.get("version", "pkg")
        self.svr_online_pkg_md5_ = parser.get("version", "pkg_md5")
        ...
        return
    if self.svr_online_version_ == self.svr_version_ and self.svr_online_pkg_md5_ == self.svr_pkg_md5_ and  not self.is_svr_update_tag_:
        self.is_update_pkg_ = False
    ...
    print("Pkg is changed:%s; env is changed:%s" % (self.is_update_pkg_, self.is_update_env_))
  ...
  def download_package(self):
    """ 下载pkg的tar.gz
    """
    save_file = os.path.join(self.svr_history_pkg_path_, self.svr_pkg_file_)
    tmp_file = os.path.join(self.svr_tmp_path_, self.svr_pkg_file_)
    tmp_decrypt_file = os.path.join(self.svr_tmp_path_, "decrpyt_"+self.svr_pkg_file_)
    is_download = True
    if os.path.isfile(save_file):
      file_md5 = self._md5(save_file)
      if file_md5 <> self.svr_pkg_md5_:
          print("Backup file[%s]'s md5[%s] isn't same with specified md5[%s], remove it." % (save_file, file_md5, self.svr_pkg_md5_))
          self._remove(save_file)
      else:
        if not self.is_svr_update_tag_:
          print("Package file:%s exist, doesn't download" % self.svr_pkg_file_)
    ...
    if is_download:
      ...
      rsync_cmd=""
      if self.rsync_passwd_:
          rsync_cmd = "rsync  -vzrtopg --progress --password-file=%s  %s@%s::%s %s" % (self.rsync_passwd_file_, self.rsync_user_, self.svr_repo_, self.svr_pkg_pathfile_, tmp_file)
      else:
          rsync_cmd = "rsync  -vzrtopg --progress %s::%s %s" % (self.svr_repo_, self.svr_pkg_pathfile_, tmp_file)
      print("rsync pkg, cmd=%s" % rsync_cmd)
      ...
      if file_md5 <> self.svr_pkg_md5_:
        self.install_errmsg_ = "File[%s]'s md5[%s] isn't same with specified md5[%s]" % (self.svr_pkg_file_, file_md5, self.svr_pkg_md5_)
        self.install_success_ = False
        print(self.install_errmsg_)
        return False
      #backup
      shutil.copy(tmp_file, save_file)
    if len(self.svr_pkg_passwd_):
      #decrpty
      cmd = "openssl des3 -d -k %s -salt -in %s > %s" % (self.svr_pkg_passwd_, tmp_file, tmp_decrypt_file)
    ...
    if not self._run_cmd(remove_cmd):
      ...
      self.install_success_ = False
      return False
    if os.path.isdir(self.svr_tmp_pkg_path_):
      self._remove(self.svr_tmp_pkg_path_)
    os.mkdir(self.svr_tmp_pkg_path_)
    unzip_cmd = "tar -zxvf " + tmp_decrypt_file + " -C " + self.svr_tmp_pkg_path_ + " >/dev/null"
  ...
  def start_service(self):
    """ 启动服务
    """
    start_cmd = self.svr_script_ + " start " + self.svr_home_
  ...
  def stop_service(self):
    """ 停止服务
    """
    stop_cmd = self.svr_script_ + " stop " + self.svr_home_
  ...
  def check_service(self):
    """ check服务
    """
    check_cmd = self.svr_script_ + " check " + self.svr_home_
  ...
  def install_service(self):
    ...
    """
    install_cmd = ""
    if self.is_update_pkg_ and self.is_update_env_:
      install_cmd = self.svr_script_ + " install "  + self.svr_home_ + " all " + self.svr_tmp_pkg_path_ + " "  + self.svr_tmp_env_path_
    elif self.is_update_pkg_:
      install_cmd = self.svr_script_ + " install "  + self.svr_home_ + " pkg " + self.svr_tmp_pkg_path_
    elif self.is_update_env_:
      install_cmd = self.svr_script_ + " install "  + self.svr_home_ + " env " + self.svr_tmp_env_path_
    else:
      self.install_success_ = False
  ...
  def _export_env(self):
    os.environ['DCMD_APP_NAME'] = self.app_name_
    os.environ['DCMD_SVR_NAME'] = self.svr_name_
    os.environ['DCMD_SVR_POOL'] = self.svr_pool_
    os.environ['DCMD_SVR_VERSION'] = self.svr_version_
    os.environ['DCMD_SVR_REPO'] = self.svr_repo_ 
    os.environ['DCMD_SVR_USER'] = self.svr_user_ 
    os.environ['DCMD_SVR_IP'] = self.svr_ip_ 
    os.environ['DCMD_SVR_VERSION_MD5'] = self.svr_pkg_md5_ 
    os.environ['DCMD_SVR_VERSION_PASSWD'] = self.svr_pkg_passwd_
    os.environ['DCMD_SVR_ENV_MD5'] = self.svr_env_md5_
    os.environ['DCMD_SVR_ENV_PASSWD'] = self.svr_env_passwd_ 
    os.environ['DCMD_SVR_SCRIPT_FILE'] = self.svr_script_ 
    os.environ['DCMD_SVR_RESULT_FILE'] = self.svr_result_file_
    os.environ['DCMD_SVR_UPDATE_ENV'] = "1" if self.is_svr_update_env_  else "0"
    os.environ['DCMD_SVR_UPDATE_TAG'] = "1" if self.is_svr_update_tag_ else "0"
    os.environ['DCMD_SVR_PROCESS'] = "1" if self.is_svr_process_ else "0"
    os.environ['DCMD_SVR_ENV_V'] = self.svr_env_v_ 
    os.environ['DCMD_SVR_PATH'] = self.svr_path_ 
    os.environ['DCMD_SVR_NODE_MUTIL_POOL'] = "1" if self.svr_node_mutil_pool_  else "0"
    os.environ['DCMD_SVR_SCRIPT_FILE'] = self.svr_script_file_
    os.environ['DCMD_SYS_AGENT_PID'] = self.agent_pid_
    os.environ['DCMD_SYS_AGENT_PPID'] = self.agent_ppid_ 
...
def main():
  ...
  try:
    ...
    if not install.is_update_pkg_ and not install.is_update_env_:
