...
#                     |     |    |--image
#                     |     |    |   |--project_service_pool_version.image     
#                     |     |--tmp  --->temporary path
#                     |        |--env --->service's pool's env conf
#                     |        |--pkg --->service's package's path
#                     |        |--image --->service's image's path
#                              |--other file
import os
import commands
import shutil
...
class DcmdInstall():
  def __init__(self):
    ...
    self.is_svr_update_image_ = True if "1"== os.environ.get('DCMD_SVR_UPDATE_IMAGE') else False
    ...
    self.svr_history_image_path_  = os.path.join(self.install_path_, "history","image")
    ...
    self.svr_tmp_image_path_ = os.path.join(self.install_path_, "tmp","image")
    ...
    self.svr_image_tag_ = "%s/%s/%s" % (self.app_name_, self.svr_name_, self.svr_version_)
    self.svr_container_name_ = "%s_%s_%s" % (self.app_name_, self.svr_name_, self.svr_pool_)
    self.svr_docker_harbor_ip_ = "10.185.31.142"
    DCMD_SVR_SYS_1_mapped_port=80
    DCMD_SVR_SYS_1_port=1
    DCMD_SVR_SYS_80_mapped_port=8081
    DCMD_SVR_SYS_80_port=8080
    DCMD_SVR_SYS_POOL_GROUP=default
    DCMD_SVR_SYS_grpc_mapped_port=12333
    DCMD_SVR_SYS_grpc_port=11211
    DCMD_SVR_SYS_image_name=
    DCMD_SVR_SYS_svr_cpu=0.03
    DCMD_SVR_SYS_svr_io=1024
    DCMD_SVR_SYS_svr_mem=100
    DCMD_SVR_SYS_svr_net=100
  ...
  def init_service_env(self):
    ...
    print("Check service image path:%s" % self.svr_history_image_path_)
    self._check_and_mk_missed_path(self.svr_history_image_path_)
    
    print("Check service env path:%s" % self.svr_history_env_path_)
    self._check_and_mk_missed_path(self.svr_history_env_path_)
    
    print("Check pkg path:%s" % self.svr_tmp_image_path_)
    self._check_and_mk_missed_path(self.svr_tmp_image_path_)
    ...
    self.remove_outdate_pkg(self.svr_history_image_path_)
    return True
  ...
  def download_image(self):
    ...
    """
      ...
      pull_cmd = "docker pull %s/%s" % (self.svr_docker_harbor_ip_, self.svr_image_tag_) 
  ...
  def install_service(self):
    ...
    """
    install_cmd = ""
    if self.is_update_image_ and self.is_update_env_:
      install_cmd = self.svr_script_ + " install "  + self.svr_home_ + " all " + self.svr_tmp_pkg_path_ + " "  + self.svr_tmp_env_path_
    elif self.is_update_image_:
      install_cmd = self.svr_script_ + " install "  + self.svr_home_ + " image " + self.svr_tmp_pkg_path_
...
def main():
  ...
  try:
    ...
    if not install.is_update_iamge_ and not install.is_update_env_:
