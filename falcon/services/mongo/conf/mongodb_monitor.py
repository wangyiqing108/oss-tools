...
mongodb_hostname = "mongodb-us-west-1"
conf_filename = os.path.dirname(sys.path[0]) + '/conf/mongomon.conf'
...
for mongodb_ins in mongodb_items:
        ...
        err,conn = mongodb_monitor.mongodb_connect(host=socket.gethostname(), port=mongodb_ins["port"], db=mongodb_ins["db"], user=mongodb_ins["user"], password=mongodb_ins["password"])
