...
# Glance Port setting
openstack-config --set /etc/nova/nova.conf DEFAULT glance_api_servers $MASTER_NAME:9292
...
openstack-config --set /etc/nova/nova.conf DEFAULT novncproxy_base_url http://$MASTER_IP:6080/vnc_auto.html
...
openstack-config --set /etc/nova/nova.conf DEFAULT neutron_url http://$MASTER_NAME:9696
openstack-config --set /etc/nova/nova.conf DEFAULT neutron_admin_tenant_name service
openstack-config --set /etc/nova/nova.conf DEFAULT neutron_auth_strategy keystone
openstack-config --set /etc/nova/nova.conf DEFAULT neutron_admin_auth_url http://$MASTER_NAME:5000/v2.0
...
# database
openstack-config --set /etc/nova/nova.conf database connection mysql://$NOVA_DB_NAME:$NOVA_DB_PWD@mysqlserver:$NOVA_DB_PORT/nova

# keystone_authtoken
openstack-config --set /etc/nova/nova.conf keystone_authtoken auth_host $MASTER_NAME
...
openstack-config --set /etc/neutron/neutron.conf DEFAULT nova_url http://$MASTER_NAME:8774/v2

openstack-config --set /etc/neutron/neutron.conf DEFAULT nova_admin_username nova
openstack-config --set /etc/neutron/neutron.conf DEFAULT nova_admin_password $NOVA_USER_PWD

openstack-config --set /etc/neutron/neutron.conf DEFAULT neutron_admin_auth_url http://$MASTER_NAME:5000/v2.0
openstack-config --set /etc/neutron/neutron.conf DEFAULT send_events_interval 2

# keystone_authtoken
openstack-config --set /etc/neutron/neutron.conf keystone_authtoken auth_uri http://$MASTER_NAME:5000
openstack-config --set /etc/neutron/neutron.conf keystone_authtoken identity_uri http://$MASTER_NAME:35357
openstack-config --set /etc/neutron/neutron.conf keystone_authtoken auth_host $MASTER_NAME
...
# database
openstack-config --set /etc/neutron/neutron.conf database connection mysql://$NEUTRON_DB_NAME:$NEUTRON_DB_PWD@mysqlserver:$NEUTRON_DB_PORT/neutron
...
openstack-config --set /etc/neutron/plugins/ml2/ml2_conf.ini ovs bridge_mappings physnet$PLUGIN_ID:$NODE_BR_NIC
...
ovs-vsctl add-br $NODE_BR_NIC
if [ "$NODE_PUB_NIC" == "bond1" ];then
    ovs-vsctl add-bond $NODE_BR_NIC $NODE_PUB_NIC eth2 eth3 bond_mode=balance-tcp lacp=active
    ovs-appctl bond/show bond1
    ovs-appctl lacp/show bond1
elif [ "$NODE_PUB_NIC" == "eth1" ];then
    ovs-vsctl add-port $NODE_BR_NIC $NODE_PUB_NIC
fi
